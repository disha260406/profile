...
async function loadSavedAvatar() {
  ...
  try {
    const response = await fetch('/api/current-avatar');
    const result = await response.json();
    const savedImage = result.imageUrl || DEFAULT_AVATAR;
    avatarPhoto.src = savedImage;
  } catch (error) {
    avatarPhoto.src = DEFAULT_AVATAR;
  }
}

if (avatarPhoto) {
  const hideMissingAvatar = () => {
    if (!avatarPhoto.complete || avatarPhoto.naturalWidth === 0) {
      avatarPhoto.style.display = 'none';
      avatarPhoto.parentElement.style.background = 'rgba(168, 85, 247, 0.08)';
    } else {
      avatarPhoto.style.display = 'block';
      avatarPhoto.parentElement.style.background = 'linear-gradient(135deg, var(--bg2), var(--bg3))';
    }
  };

  avatarPhoto.addEventListener('error', hideMissingAvatar);
  avatarPhoto.addEventListener('load', hideMissingAvatar);
  loadSavedAvatar();
}
...
if (profileUpload && avatarPhoto) {
  profileUpload.addEventListener('change', async (event) => {
    ...
    try {
      ...
      avatarPhoto.src = result.imageUrl;
      avatarPhoto.style.display = 'block';
      if (avatarPhoto.parentElement) {
        avatarPhoto.parentElement.style.background = 'linear-gradient(135deg, var(--bg2), var(--bg3))';
      }
    } catch (error) {
...
window.setAvatarByUrl = async function(url) {
  try {
    const response = await fetch('/api/set-avatar-url', {
      method: 'POST',
    ...
    const result = await response.json();
    if (!response.ok) throw new Error(result.message || 'Could not save image URL');
...
if (contactForm) {
  contactForm.addEventListener('submit', (e) => {
    ...
    const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent('dishadevgirkar26@gmail.com')}&su=${subject}&body=${body}`;

    if (button) {
      button.innerHTML = '<i class="fas fa-check"></i> Opening Gmail...';
